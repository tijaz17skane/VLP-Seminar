...
from methods.utils.yolo_loss import YOLOLoss
from torchmetrics.detection.mean_ap import MeanAveragePrecision
from methods.utils.detection_utils import non_max_suppression
from pytorch_lightning import LightningModule
torch.use_deterministic_algorithms(True, warn_only=True)
torch.use_deterministic_algorithms(False)


class FinetuneDetector(LightningModule):
    def __init__(self, img_encoder, config):
        super().__init__()
        self.save_hyperparameters()
        self.config = config
        self.model = ModelMain(img_encoder)
        # Extract values from the config dictionary
        self.learning_rate = config["det"].get("lr", 5e-4)
        self.weight_decay = config["det"].get("weight_decay", 1e-6)
        self.imsize = config["dataset"].get("imsize", 224)
        self.conf_thres = config["det"].get("conf_thres", 0.5)
        self.iou_thres = config["det"].get("iou_thres", [0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75])
        self.nms_thres = config["det"].get("nms_thres", 0.5)
        self.yolo_losses = []
        for i in range(3):
            self.yolo_losses.append(YOLOLoss(self.model.anchors[i], self.model.classes,
                                             (self.imsize, self.imsize)))
        self.val_map = MeanAveragePrecision(
            iou_thresholds=self.iou_thres)
